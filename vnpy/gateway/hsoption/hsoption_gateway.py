...
from vnpy.trader.utility import get_folder_path, print_dict
...
class HsoptionGateway(BaseGateway):
    ...
    def __init__(self, event_engine, gateway_name="HSOPTION"):
        """Constructor"""
        super().__init__(event_engine, gateway_name)

        self.td_api = TdApi(self)
        self.md_api = SoptMdApi(self)
        self.user_upper_name = "VNPY"

    def connect(self, setting: dict) -> None:
        """"""
        td_userid = setting["交易用户名"]
        td_password = setting["交易密码"]
        self.user_upper_name = setting.get('用户名称拼音大写', 'VNPY')
...
class TdApi:
    ...
    def generate_op_station(self):
        ...
        try:
            ...
            for disk in c.Win32_DiskDrive():
                if disk.SerialNumber is None:
                    continue
                hd = disk.SerialNumber.strip().replace(',', '').replace(".","")
                for partition in disk.associators("Win32_DiskDriveToDiskPartition"):
                    for logical_disk in partition.associators("Win32_LogicalDiskToPartition"):
                        vol = "^" + str(logical_disk.VolumeSerialNumber).replace(":","")
                        break
        except Exception as ex:
            if len(hd) == 0:
                hd = "W-DXW1D5AR07323"
            if len(vol) == 0:
                vol = '^C^2ADBE693^NTFS^475.66'
        try:
            for disk in c.Win32_LogicalDisk(DriveType=3):
                pi = "^".join([str(disk.Caption).replace(':', ""),
                               str(disk.FileSystem),
                               str(round(int(disk.Size) / (1024 * 1024 * 1024), 2))+'G'])
        ...
        op_station = f"PC;IIP={iip};IPORT=53039;LIP={lip};MAC={mac};HD={hd};PCN={pcn};CPU={cpu};PI={pi};VOL={vol}@{software};VPN;QQ-{self.gateway.user_upper_name}"
    ...
    def login(self) -> int:
        ...
        #self.gateway.write_log('{}:{}'.format(FUNCTION_USER_LOGIN, print_dict(req)))
    ...
    def send_order(self, req: OrderRequest) -> int:
        ...
        #self.gateway.write_log('{}:{}'.format(FUNCTION_SEND_ORDER, print_dict(td_req)))
